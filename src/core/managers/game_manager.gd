...
signal state_changed(old_state: GameConstants.GameState, new_state: GameConstants.GameState)
...
var _current_state: GameConstants.GameState = GameConstants.GameState.MENU
var _previous_state: GameConstants.GameState = GameConstants.GameState.MENU


func _ready() -> void:
	# Load initial scene (deferred to allow scene tree to finish initialization)
	if not initial_scene.is_empty():
		GameServices.scenes.change_scene_immediate.call_deferred(initial_scene)
...
func get_state() -> GameConstants.GameState:
	return _current_state


## Get the previous game state
func get_previous_state() -> GameConstants.GameState:
	return _previous_state


## Pause the game
func pause_game() -> void:
	if _current_state == GameConstants.GameState.PLAYING:
		get_tree().paused = true
		set_state(GameConstants.GameState.PAUSED)
		GameServices.events.game_paused.emit()
		
		if pause_menu_enabled and not pause_menu_scene.is_empty():
			GameServices.scenes.push_scene(pause_menu_scene)


## Resume the game
func resume_game() -> void:
	if _current_state == GameConstants.GameState.PAUSED:
		get_tree().paused = false
		set_state(_previous_state)
		GameServices.events.game_resumed.emit()


## Quit the game with optional confirmation
func quit_game() -> void:
	if quit_confirmation_enabled and _current_state == GameConstants.GameState.PLAYING:
...
func return_to_menu() -> void:
	...
	GameServices.events.returned_to_menu.emit()
	
	if not initial_scene.is_empty():
		GameServices.scenes.change_scene(initial_scene)
...
func _toggle_fullscreen() -> void:
	...
	if current_mode == DisplayServer.WINDOW_MODE_FULLSCREEN:
		...
		GameServices.settings.set_setting("display", "window_mode", 0)
	else:
		DisplayServer.window_set_mode(DisplayServer.WINDOW_MODE_FULLSCREEN)
		GameServices.settings.set_setting("display", "window_mode", 1)


func _toggle_debug_mode() -> void:
	# Emit custom event for debug overlay to handle
	GameServices.events.emit_custom_event("debug_toggled", {})
