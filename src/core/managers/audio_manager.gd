...
func set_bus_volume(bus_name: String, volume: float) -> void:
	...
	# Save to settings
	if GameServices.settings:
		GameServices.settings.set_setting("audio", bus_name.to_lower() + "_volume", volume)
...
func _load_volumes_from_settings() -> void:
	if not GameServices.settings:
		return
	
	var buses = [BUS_MASTER, BUS_MUSIC, BUS_SFX, BUS_UI, BUS_AMBIENCE, BUS_VOICE]
	for bus in buses:
		var volume = GameServices.settings.get_setting("audio", bus.to_lower() + "_volume", 1.0)
